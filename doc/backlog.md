...
>
> Last updated: 2026-05-26, end of Round 46 — Block feature SHIPPED end-to-end (Steps 242-244 backend + UI + close-out), Limit + Whitelist both PERMANENTLY REMOVED (Steps 244 + 245) after honest evaluation. Round 46 was the project's first complete verify-first saga: 5-min ssh batch upfront killed Limit + locked Block path, 3 hotfixes (242a/b/c) all caught in backend phase before any UI commit, UI shipped clean. Round 47 grooming in progress — `doc/luci-theme-toolbox.md` extended with 6 new sections (rpcd write methods, nftables.d semantics, atomic-replace prelude, BusyBox-safe shell, verify-first workflow, LuCI modal pattern), INDEX.md current-state synced, this backlog file rewritten for Round 46 close-out. Memory now 10 entries (added `verify-first-implement-second` in Step 242c). **Round 48 = user-driven; no carryover backlog from Round 46**.
...
- **Whitelist** — 🪦 **REMOVED PERMANENTLY** (Step 245, 2026-05-26, Round 46 close-out). Honest evaluation after Block shipped found 4 disqualifying reasons: (1) modern MAC randomization (iOS / Android 10+ / macOS) defeats the mechanism, blocking legitimate device reconnects; (2) security ≈ 0 — anyone with a passive sniffer can spoof a whitelisted MAC, WPA3 password is the real gate; (3) functional duplicate of Block in inverse form with 100× maintenance burden (every new family device needs explicit allow); (4) LuCI ships this already at `Network → Wireless → <SSID> → MAC Filter` — duplicating is anti-pattern. Like Limit, the UI placeholder accumulated as deferred-indefinitely drift; Step 245 deletes it outright. Whitelist is **NOT a future-round candidate** unless the user surfaces a specific use case the standard LuCI mac_filter can't handle.
- **Limit** — 🪦 **REMOVED PERMANENTLY** (Step 244, 2026-05-25). Backend never built; UI button removed entirely in Step 244 after Step 243 user feedback ("去掉吧"). Round 46 verification had already disqualified the feature (tc not in IW24.10 default; nft `limit rate over` is policing-not-shaping with TCP retransmit-heavy UX + structural SFO bypass vulnerability). The Step 243 transitional "Limit (toast: install sqm-scripts)" was deemed noise; outright removal is cleaner.
- **Block** — ✅ **SHIPPED 2026-05-25**, Round 46 close-out. nft inet table `design_x` with `blocked_macs` set + forward/input/output drop hooks at `/usr/share/nftables.d/ruleset-post/design_x.nft`. Persistence file regenerated on every Block/Unblock action via rpcd handler. UI is devices.js Clients-card Block/Unblock toggle with confirmation modal on the destructive direction, red-dot indicator + Status badge on blocked rows. **End-to-end verified on user's router** (post-242c): Wi-Fi stays connected, internet traffic dropped, fw4 reload preserves state, unblock restores instantly. **Shipped across 5 Steps**: 242 (backend+ACL), 242a (path hotfix), 242b (atomic-replace prelude), 242c (paste→shell-loop hotfix), 243 (UI wiring), 244 (Limit button removed).
