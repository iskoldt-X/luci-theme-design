...
- **Block** — ✅ **IN PROGRESS, Step 242 + Step 242a (this round)**. nft inet table `design_x` with `blocked_macs` set + forward/input/output drop hooks. Standalone table at `/usr/share/nftables.d/ruleset-post/design_x.nft` (Step 242a fixed the path — Step 242 had used `table-post/` which fw4 includes INSIDE its main table block, breaking our `table ... {}` wrapper). rpcd handler adds 3 methods (`list-blocks` / `block-mac` / `unblock-mac`) with strict server-side MAC regex validation (Step 153 lesson). Persistence file regenerated on every write. **Step 242 = backend + verification clip; Step 242a = hotfix dir path; Step 243 = UI wiring (devices.js Block button) after user reverifies**.
